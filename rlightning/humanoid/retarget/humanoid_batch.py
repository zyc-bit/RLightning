...
class HumanoidBatch:
    ...
    def __init__(self, cfg: HumanoidBatchCfg, device: str = "cpu"):
        """Construct a humanoid batch instance.

        Args:
            cfg (HumanoidBatchCfg): Configuration for initialization.
            device (str, optional): Device name. Defaults to "cpu".
        """

        self.cfg = cfg
        self.mjcf_file = cfg.asset_file_path
        ...
        mjcf_data = mjcf_loader.load_mjcf(self.mjcf_file, self.num_dof, device)
        ...
        self.mjcf_data = mjcf_data

        motors_not_in_joints = set(motors) - set(joints)
        if len(motors_not_in_joints) > 0:
            logging.warning(
                f"Motors {motors_not_in_joints} are not in the joints list. "
                "This may cause issues with the model."
            )

        # NOTE: Check if the first joint is a free joint
        #   If it is, we will treat it differently
        #   If it is not, we will treat it as a regular joint
        #   If there is no joint, we will treat it as a regular joint
        #   This is to ensure compatibility with different MJCF files
        #   and to ensure that the model can be used with different humanoid models
        ...
        if cfg.extend_configs is not None:
            tmp_extend_parents = []
            tmp_extend_offsets = []
            tmp_extend_local_rotations = []
            tmp_extend_body_names = []

            for extend_config in cfg.extend_configs:
                tmp_extend_body_names.append(extend_config.joint_name)
                tmp_extend_parents.append(self.body_names.index(extend_config.parent_name))
                tmp_extend_offsets.append(extend_config.pos)
                tmp_extend_local_rotations.append(extend_config.rot)

            self.num_extend_dof += len(cfg.extend_configs)

            tmp_extend_parents = torch.tensor(
                tmp_extend_parents, dtype=self._parents.device, device=device
            )
            tmp_extend_offsets = torch.tensor(
                tmp_extend_offsets, dtype=self._offsets.dtype, device=device
            ).unsqueeze(0)
            tmp_extend_local_rotations = torch.tensor(
                tmp_extend_local_rotations, dtype=self._local_rotation.dtype, device=device
            ).unsqueeze(0)

            self._parents = torch.cat([self._parents, tmp_extend_parents], dim=0)
            self._offsets = torch.cat([self._offsets, tmp_extend_offsets], dim=1)
            self._local_rotation = torch.cat(
                [self._local_rotation, tmp_extend_local_rotations], dim=1
            )
            self.body_names_augment += tmp_extend_body_names

        self.num_bodies = len(self.body_names)
        self.num_bodies_augment = len(self.body_names_augment)

        self.joints_range = mjcf_data["joints_range"].to(device)
        self._local_rotation_mat = common.quaternion_to_matrix(
    ...
    def fk_batch(self, pose, trans, convert_to_mat=True, return_full=False, dt=1 / 30):
        ...
        pose_all = common.insert_tensors_at_dim(
            pose,
            indices=body_no_joint_idx,
            dim=-2,
            values=torch.zeros(body_no_joint_shape, dtype=dtype, device=device),
        )
        ...
        if convert_to_mat:
            pose_quat = common.axis_angle_to_quaternion(pose_all.clone())
            pose_mat = common.quaternion_to_matrix(pose_quat)
        else:
            pose_mat = pose_all
        ...
        wbody_rot = common.wxyz_to_xyzw(common.matrix_to_quaternion(wbody_mat))
        ...
        if return_full:
            ...
            return_dict.local_rotation = common.wxyz_to_xyzw(pose_quat)
