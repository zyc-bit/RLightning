#!/bin/bash
# Launch distributed RSL-RL training for IsaacLab with Ray.
# Expects: a local `.venv` under `examples/rslrl_isaaclab`.
...
ray stop
ray start --head --num-gpus=$GPU_COUNT --dashboard-host 0.0.0.0

cd $WORK_DIR

TRAINING_CONFIG="launch.yaml"
