...
def bad_anchor_pos(env: ManagerBasedRLEnv, command_name: str, threshold: float) -> torch.Tensor:
    ...
    return torch.norm(command.motion.anchor_pos_w - command.robot_anchor_pos_w, dim=1) > threshold
...
def bad_anchor_pos_z_only(
    ...
    command: MotionCommand = env.command_manager.get_term(command_name)
    return (
        torch.abs(command.motion.anchor_pos_w[:, -1] - command.robot_anchor_pos_w[:, -1])
        > threshold
    )
...
def bad_anchor_ori(
    ...
    motion_projected_gravity_b = math_utils.quat_rotate_inverse(
        command.motion.anchor_quat_w, asset.data.GRAVITY_VEC_W
    )
