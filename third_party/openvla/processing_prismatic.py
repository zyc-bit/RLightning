...
# Original path: third_party/openvla/processing_prismatic.py
...
class PrismaticImageProcessorForBatch(PrismaticImageProcessor):
    def apply_transform(self, img: torch.Tensor) -> torch.Tensor:
        """
        Apply `functional` variant of TIMM's Transform = Compose([Resize -> CenterCrop -> ToTensor -> Normalize])
...
class PrismaticProcessorForBatch(PrismaticProcessor):
    attributes: ClassVar[List[str]] = ["image_processor", "tokenizer"]
    image_processor_class: str = "AutoImageProcessor"
    tokenizer_class: str = "AutoTokenizer"
